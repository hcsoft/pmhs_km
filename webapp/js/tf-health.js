				...
				Ext.Panel,
				{
					...
					f_add : function(isSlient) {
						...
						if (getParams == '-1') {
							...
							showInfoObj.Infor(selections[0].data.name
									+ '小朋友已经建立了保健手册');
						} else {
									...
									if (data.data == 'quit') {
										...
										if (d.indexOf(':') > 0) {
											...
											if (retVal[0] == 'retId') {
												...
												Ext
														.getCmp(
																'woman_generalItems_tabpanel')
														.remove(
																'firstvisit_tabpanel_id');
												Ext
														.getCmp(
																'woman_generalItems_tabpanel')
														.remove(
																'visitBeforeBorn_tabpanel_id');
												Ext
														.getCmp(
																'woman_generalItems_tabpanel')
														.remove(
																'visitAfterBorn_tabpanel_id');
												Ext
														.getCmp(
																'woman_generalItems_tabpanel')
														.remove(
																'visitAfterBorn42_tabpanel_id');
												Ext
														.getCmp(
																'woman_generalItems_tabpanel')
														.add(
					...
					createActions : function() {
						var searchCondition = [ [ 'a.name', '姓名' ],
								[ 'c.highRisk', '高危筛选' ],
								[ 'a.inputDate', '录入日期' ],
								[ 'a.lastModifyDate', '修改日期' ],
								[ 'b.birthday', '出生日期' ],
								[ 'a.fileNo', '档案编码' ],
								[ 'b.idnumber', '身份证号' ],
								[ 'b.linkman', '联系人' ],
								[ 'a.paperFileNo', '纸质档案号' ],
								[ 'b.workUnit', '工作单位' ], [ 'a.nation', '国籍' ],
								[ 'b.farmStatus', '是否农业户口' ],
								[ 'b.townStatus', '是否城镇户口' ],
								[ 'a.address', '地址' ],
								[ 'a.residenceAddress', '户籍地址' ] ];
						if (this.isAlreadyMaternal) {
							searchCondition = [ [ 'a.name', '姓名' ],
									[ 'b.birthday', '出生日期' ],
									[ 'a.fileNo', '档案编码' ],
									[ 'b.idnumber', '身份证号' ],
									[ 'a.paperFileNo', '纸质档案号' ],
							...
							if (time < 20000) {
								if (!this.menu.getNodeById(node)) {
									if (this.menu.getNodeById(parent)) {
										this.menu.expandPath(this.menu
												.getNodeById(parent).getPath());
										setTimeout(function() {
											that.waitnodeexpend(node, node
													.substring(0,
															parent.length + 3),
													time + 500)
										}, 500);
										...
										setTimeout(function() {
											that.waitnodeexpend(node, parent,
													time + 500)
										}, 500);
									}
								} else {
									this.menu.selectPath(this.menu.getNodeById(
											node).getPath());
								}
							}
						};
							...
							printHealthFile = new Ext.Action({
								...
								handler : function() {
									...
									if (selections.length == 1) {
										...
										PrintHealthFileAndExamClass
												.printHealthFileOther(fileNo);
									} else {
										...
										handler : function() {
											...
											if (selections.length == 1) {
												...
												if (this.womanPrintType == '1') {
													PrintHealthFileAndExamClass
															.printFirstVisitExam(id);
												} else if (this.womanPrintType == '2') {
													PrintHealthFileAndExamClass
															.printVisitBeforeBornExam(id);
												} else if (this.womanPrintType == '3') {
													PrintHealthFileAndExamClass
															.printVisitAfterBornExam(id);
												} else if (this.womanPrintType == '4') {
													PrintHealthFileAndExamClass
															.printVisitAfterBornExam42(id);
												}
											} else {
												showInfoObj
														.Infor('请选择打印的孕产妇随访记录！');
									...
									handler : function() {
										...
										if (selNode) {
											...
											Ext.getCmp(id).getEl().mask(
													'导出数据加载中...');
											var filterKey = this.combo
													.getValue();
											var filterValue = this.filterField
													.getValue();
															...
															new Ext.Action(
																	{
																		...
																		handler : function() {
																			...
																			if (selections.length > 0) {
																				UserMenuTreeService
																						.getSelectedUserDistrict(function(
																								data) {
																							MethodObj
																									.openSelectedDistrictWin(
																											selections,
																											data);
																						});
																			} else {
						...
						if (this.isMaternal) {
							...
							}), '-', this.combo, this.filterField,
									this.onlyOrgField, this.orgcombo,
									new Ext.Action({
										text : '查询',
										iconCls : 'c_query',
										handler : function() {
							...
							funcAction = [
									...
									new Ext.Action(
											...
											}), '-', this.combo01, this.combo,
									this.filterField, this.onlyOrgField,
									this.onlySelfField, new Ext.Action({
										text : '查询',
										iconCls : 'c_query',
										handler : function() {
											this.load(true);
							...
							funcAction.push('-');
							funcAction.push(new Ext.form.Label({
								xtype : 'lable',
								text : '选择机构'
							}));
					...
					getParams : function() {
						...
						if (selNode) {
							...
							var params = {};
							console.log("onlyself ==" + this.onlySelfField
									&& this.onlySelfField.getValue())
							if (this.onlySelfField
									&& this.onlySelfField.getValue()) {
								params['onlyself'] = this.onlySelfField
										.getValue();
							}
							if (this.noExamField && this.noExamField.getValue()) {
								params['noexam'] = this.noExamField.getValue();
							}
							console.log("onlyorg ==" + this.onlyOrgField
									&& this.onlyOrgField.getValue())
							if (this.onlyOrgField
									&& this.onlyOrgField.getValue()) {
								params['onlyorg'] = this.onlyOrgField
										.getValue();
							}
							if (this.orgcombo) {
